(function () {
  ...
  function renderIceCream(iceCreams) {
    ...
    if (window.innerWidth > 610) {
      const secondSlide =
        currentSlide + 1 >= iceCreams.length ? n++ : currentSlide + 1;
      slideContainer.innerHTML += iceCreams[secondSlide];
    }
    if (window.innerWidth > 880) {
      const thirdSlide =
        currentSlide + 2 >= iceCreams.length ? n++ : currentSlide + 2;
      slideContainer.innerHTML += iceCreams[thirdSlide];
    }
    if (window.innerWidth > 1180) {
      const fourSlide =
        currentSlide + 3 >= iceCreams.length ? n++ : currentSlide + 3;
